...
def update_row(row):
    """
    Used in lambda function to update values for job title
    @param row: row of dataframe
    @return: updated row it there is something to update
    """
    if row['job_title_2'] != row['job_title']:
        row.history = row.history + f',{row.job_title_2}'
        row.updated_at = row.updated_at_2
        row.job_title = row.job_title_2
        row.source_table = row.source_table_2
    return row


def update_users_jobs() -> bool:
    """
    Update users_jobs tables in UsersJobs and Segmentation_V2 datasets
    ...
    if final_df_to_upload.shape[0] > 0:
    	...
    	update_metadata(project_id, POSTGRE_AUTH, processed_job_df,"Segmentation_v2", table)
