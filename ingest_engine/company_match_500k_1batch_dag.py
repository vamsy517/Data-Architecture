...
def get_match():
    file = 'gd_wishlist.xlsx'
    n = 10000
    n_closest = 7
    threshold = 'None'
    df_all = pd.read_excel('/home/ingest/company_match_500k_materials/32k/'+file)
    ...
    for g, df in df_all.groupby(np.arange(len(df_all)) // n):
        ...
        if Path(f"/home/ingest/company_match_500k_materials/32k_results/{file.split('.')[0]}/"+file_name).exists():
        ...
        matched_df.to_excel(f"/home/ingest/company_match_500k_materials/32k_results/{file.split('.')[0]}/"+file_name)
...
with DAG('company_match_500k_1batch_dag',
         catchup=False,
