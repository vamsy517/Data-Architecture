...
def collect_all_emails_ids():
    ...
    emails = emails[emails['value'] != 'ravi.tangirala@progressivemediagroup.in']
    emails = emails[emails['value'] != 'ravikumar.tangirala@globaldata.com']
    emails = emails[emails['value'] != 'sam.hall@globaldata.com']
...
def fix_log_error():
    ...
    with open ('/home/ingest/logs/update_permutive_pardot_match.csv', 'a') as fl:
        fl.write(f'[{str(max_date)} , {str(min_date)}]')
    return True 

default_args = {
    'owner': 'project-pulldata_permutive_pardot_match_update',
    'start_date': datetime.datetime(2021, 2, 9, 21, 1, 00),
    'concurrency': 1,
    'retries': 0
}

with DAG('project-pulldata_permutive_pardot_match_update',
     catchup=False,
