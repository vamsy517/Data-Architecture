...
from users_jobs_update.gbq_queries import QUERY_GET_ALL_JOBS, GET_PREV_DATA
from users_jobs_update.utils import update_row


def update_users_jobs() -> bool:
    """
        Upload Combined Jobs data to GBQ
    """
    ...
    list_symbols = ['', ' ', '.', '-', ',', '..', '...', '--', '/', '?', '???', 'TEST', 'TEst', 'Test', 'test',
                    'x', 'xx', 'xxx', 'xx', 'NULL', 'none', 'None', 'NONE'
                    'xxxx']
    ...
    upload_table_to_gbq(project_id, final_df_to_upload, dataset, table, method='replace')
    # Update metadata
    update_metadata(project_id, POSTGRE_AUTH, final_df_to_upload, dataset, table)
    print('New record: ' + str(final_df_new.shape[0]))
    print('Update record: ' + str(len(merged_df.query('job_title_2 != job_title'))))
