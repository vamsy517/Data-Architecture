...
def log_update(ids_clicks, ids_details):
    ...
    with open ('/home/ingest/logs/update_pardot_emails_results_b2c.csv', 'a') as fl:
        curr_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        ids_clicks_count = str(len(ids_details))
...
def upload_data():
    ...
    max_created_updated = get_max(project_id, 'PardotB2C', 'EmailClicks')
    ...
    for c in chunks:
        ...
        update_list = datahub_updater.Updater('project-pulldata', 
                                            'PardotB2C', 'EmailClicks', c,
                                            'id', ['id'],
                                            ids_details)
        print('Update Pardot EmailsClicks data')
        update_list.df['list_email_id'] = update_list.df['list_email_id'].astype(pd.Int64Dtype())
...
default_args = {
    'owner': 'project-pulldata_Pardot_Emails_data_update_B2C',
    'start_date': datetime.datetime(2020, 11, 24, 21, 00, 00),
    'concurrency': 1,
    'retries': 0
}

with DAG('project-pulldata_Pardot_Emails_data_update_B2C',
     catchup=False,
