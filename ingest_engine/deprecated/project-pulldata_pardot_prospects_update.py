...
def log_update(ids_update, ids_new, table):
    ...
    with open ('/home/ingest/logs/update_prospects_results.csv', 'a') as fl:
...
def upload_data():
    ...
    old_df = get_table(project_id, 'Pardot', 'Prospects')
    ...
    # drop these IDs from final dataframe
    final_df = final_df[~final_df.id.isin(ids_update)]
    # concat new updated data from API with dataframe with final data and drop duplicates
    final_df = pd.concat([final_df, new_updated_df], axis=0, ignore_index=True, sort=False)
    ...
    # upload data to gbq
    final_df.to_gbq(destination_table='Pardot.Prospects',
              project_id = project_id, if_exists = 'append')
    print('Table Prospects uploaded successfully to GBQ.')
    # upload data to postgres
    final_df.to_sql('Prospects',schema='pardot', con=postgre_engine,method='multi',
              chunksize = 10000, if_exists = 'append')
    print('Table Prospects uploaded successfully to Postgre.')
    # update metadata
    update_metadata(final_df, 'Pardot', 'Prospects', postgre_engine, gbq_client, project_id) 
    # record log file
