...
class CommonTradeAPI():
    ...
    def tables_create_countrycodes(self, table_checks):
        ...
        countrycodes = get_request(url='https://comtrade.un.org/Data/cache/reporterAreas.json').json()['results']
    ...
    def tables_create_traderegimes(self, table_checks):
        ...
        trade_regimes = get_request(url='https://comtrade.un.org/Data/cache/tradeRegimes.json').json()['results']
    ...
    def get_data_availability(self):
        ...
        for type_ in ['C','S']:
            for freq in ['A','M']:
                ...
                try:
                    data_availability = get_request(url='http://comtrade.un.org/api//refs/da/view?', params = get_data_params).json()
                    ...
                    continue
                sleep(1)
        if len(self.data_availability)>0:
            from sklearn.utils import shuffle
            self.data_availability = concat(self.data_availability, axis=0, ignore_index=True)
            self.data_availability = shuffle(self.data_availability)
            self.data_availability = self.data_availability[self.data_availability['px'].isin(['H5','EB02','HS'])]
            self.data_availability.sort_values(by='ps', ascending=False, inplace=True)
            self.data_availability.reset_index(drop=True, inplace=True)

            logpr('Number of raw requests: {}'.format(len(self.data_availability)))
            print('')
        

        ######## CAN CREATE FILTERING RULES TO AVOID DOWNLOADING AVAILABLE DATA AGAIN
    ...
    def get_comtrade_data(self):
        ...
        for index,row in self.data_availability.iterrows():
            reporter_data = []
