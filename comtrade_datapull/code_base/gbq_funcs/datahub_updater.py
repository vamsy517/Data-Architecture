...
from .gbq_creds import gbq_credentials
...
class Updater():
    ...
    def __init__(self, project_id, dataset, table, df, entity_id_col, ids_datapoints, ids_entity, gbq_creds_path):
        ...
        self.gbq_creds_path = gbq_creds_path
    ...
    def update_data(self):
        ...
        df_existing = self.download_existing_data(self.dataset, self.table, self.entity_id_col, self.ids_entity, self.gbq_creds['bqclient'], self.gbq_creds['bqstorageclient'])
        ...
        postgre_engine = create_engine('postgresql+psycopg2://postgres:test@104.155.153.113:5432/postgres')
        self.gbq_creds = gbq_credentials(filename=self.gbq_creds_path)
        self.delete_data(self.ids_entity, self.dataset, self.table, self.entity_id_col, postgre_engine, self.gbq_creds['bqclient'])
        self.upload_data(df_to_upload, self.dataset, self.table, postgre_engine, self.project_id)
        self.update_metadata(df_to_upload, self.dataset, self.table, postgre_engine, self.gbq_creds['bqclient'], self.project_id)
        return True