...
clearbit_1 = pd.read_excel('clearbit_names_1.xlsx', index_col=0)
clearbit_2 = pd.read_excel('clearbit_names_2.xlsx', index_col=0)
clearbit_1_fix = pd.read_excel('clearbit_names_1_fix.xlsx', index_col=0)
clearbit_2_fix = pd.read_excel('clearbit_names_2_fix.xlsx', index_col=0)
clearbit_all = list(clearbit_1.name)
clearbit_all.extend(list(clearbit_2.name))
df_fixname = list(clearbit_1_fix.name)
df_fixname.extend(list(clearbit_2_fix.name))
clearbit_all = [item if type(item) == str else str(item) for item in clearbit_all]
...
def get_distance(df_names, n_closest, threshold, clearbit_all, df_fixname):
    ...
    def get_closest_names(company_name,  clearbit_all, df_fixname, n_closest, threshold):
        '''
            Input:
    ...
    x['name'],  clearbit_all, df_fixname, n_closest, threshold), axis = 1)
    # copy the dataframe
...
file_upload.add_argument('excel_file',
                         type=werkzeug.datastructures.FileStorage, 
                         location='files', 
                         required=True, 
                         help='Upload .xlsx file with company names.<br/>(File must be one sheet with one column with header "name" and Company names below.)')
file_upload_time = reqparse.RequestParser()
file_upload_time.add_argument('excel_file',
                         type=werkzeug.datastructures.FileStorage, 
                         location='files', 
                         required=True, 
                         help='Upload .xlsx file with company names.<br/>(File must be one sheet with one column with header "name" and Company names below.)')
...
# finction to check if the file that try to upload is in correct format( extension is .xlsx)
...
#         args['doc1'].save(os.path.join(app.config['Upload_folder'],secure_filename(args['doc1'].filename)))
        ...
        excel_file = args['excel_file'].stream
        # check if the file is in correct format
        if not allowed_file(args['excel_file'].filename):
            return jsonify('File is not in .xlsx format')
...
#         args['doc1'].save(os.path.join(app.config['Upload_folder'],secure_filename(args['doc1'].filename)))
        ...
        if not allowed_file(args['excel_file'].filename):
            return jsonify('File is not in .xlsx format')
        # create dataframe from uploaded excel file
        df_names = pd.read_excel(excel_file)
        # check if the content of the datafame is one cloumn with header 'name'
        # if not return error message
        if not(df_names.shape[1] == 1 and df_names.columns[0] == 'name'):
        ...
        if threshold.isdigit() or threshold == 'None':
            ...
            copy_wishlist_df = get_distance(df_names, n_closest, threshold, clearbit_all, df_fixname)
        else:
            return jsonify('Please provide intiger number for threshold or None')
        # create response for download the excel file with the generated matches
        in_memory_fp = BytesIO()
        copy_wishlist_df.to_excel(in_memory_fp)
        in_memory_fp.seek(0)
        resp = make_response(send_file(in_memory_fp, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"))
        resp.headers["Content-Disposition"] = "attachment; filename=generated_names.xlsx" 
